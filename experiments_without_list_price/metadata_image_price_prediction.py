...
def thirty_pc_acc(y_true, y_pred):
        return K.mean((K.abs(y_pred - y_true)/y_true) <= 0.3)

def twentyfive_pc_acc(y_true, y_pred):
        return K.mean((K.abs(y_pred - y_true)/y_true) <= 0.25)

def twenty_pc_acc(y_true, y_pred):
        return K.mean((K.abs(y_pred - y_true)/y_true) <= 0.2)

def fifteen_pc_acc(y_true, y_pred):
        return K.mean((K.abs(y_pred - y_true)/y_true) <= 0.15)

def ten_pc_acc(y_true, y_pred):
        return K.mean((K.abs(y_pred - y_true)/y_true) <= 0.1)

def five_pc_acc(y_true, y_pred):
        return K.mean((K.abs(y_pred - y_true)/y_true) <= 0.05)
...
def split_data(data):
	...
	#List price is very close to sold price in most cases, hence excluding it. The commented out lines include list price.
	#Xtrain = train_data[:,3:]
	Xtrain = train_data[:,4:]
	Ytrain = train_data[:,2]
	#Xtest = test_data[:,3:]
...
#Setting training and testing size
train_size = Xtrain.shape[0]
test_size = Xtest.shape[0]
num_batches = int(train_size/batch_size)

#A multi input neural network model to pass images and metadata separately
...
model.compile(optimizer='rmsprop', loss='mse',metrics=['mape', 'mae', five_pc_acc, ten_pc_acc, fifteen_pc_acc, twenty_pc_acc, twentyfive_pc_acc, thirty_pc_acc])
...
#Evaluating the model on a test set and printing out a few predictions
...
#Saving the model
model.save('metadata_images_fully_trained_model.h5')
